...
```{r}
## to keep and combine
df1<- hap1syn %>% dplyr::filter(genome == "salicifolius")
dfsal<- salsyn %>% dplyr::filter(genome == "hap1")
## to exclude and eliminate from the list
df1ex<- hap1syn %>% dplyr::filter(genome == "hap2")
dfsalex<-salsyn %>% dplyr::filter(genome == "hap2")
```
