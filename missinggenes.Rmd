...
df1auto<-filter(df1, str_detect(region,"A."))

a<-ggplot(df1auto, aes(x="", y=number, fill=name)) +
  geom_bar(stat="identity", width=1) +
  coord_polar("y", start=0) + theme_void()
b<-ggplot(df1newX, aes(x="", y=number, fill=name)) +
  geom_bar(stat="identity", width=1) +
  coord_polar("y", start=0) + theme_void()
c<-ggplot(df1oldX, aes(x="", y=number, fill=name)) +
  geom_bar(stat="identity", width=1) +
  coord_polar("y", start=0) + theme_void()

library(cowplot)
cowplot::plot_grid(a,b,c,labels = c("Autosome","newX","oldX"))
#ggsave("pie_geneloss.jpg",dpi = 300, width = 6, height = 4, units = "in")
