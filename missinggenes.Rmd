...
slr_rename_h1<-function(x){mutate(x,region=if_else(interpChr =="X" & end<=71000000,"PAR1",
...
genes missing from hap2 \#### list for blasting genes against hap2 to confirm their loss
...
#   region=if_else(interpChr=="Y1" & end<=64000000,"NewY1",
#          if_else(interpChr =="Y1" & (end>64000000 & end<=272000000),"OldY1",
#          if_else(interpChr =="Y1" & (end > 272000000 & end <= 344000000),"PAR1",
...
slr_rename_h2_fix<- function(x){mutate(x,
...
pars_xloss_fix<-slr_rename_h2_fix(hap2_xloss)

summary_xloss_pars_fix<-summarisebyregion(pars_xloss_fix,n_xloss)

total_pangenes2_fix<-slr_rename_h2_fix(hasanortho2)

summary_total_pangenes2_fix<-summarisebyregion(total_pangenes2_fix,n_total)

chrpropnPAR_xloss_fix<-full_join(summary_total_pangenes2_fix,summary_xloss_pars_fix,by="region") %>% 
   mutate(., propn=n_xloss/n_total)  %>%
   mutate(region = factor(region, levels=c("A1","A2","A4","PAR1","OldY1","NewY2","OldY2","NewY1","PAR2"))) 
## plot proportions 
print(chrpropnPAR_xloss)
print(chrpropnPAR_xloss_fix)



slr_rename_h1_fix<-function(x){mutate(x,region=if_else(chr =="X" & end<=71000000,"PAR1",
...
pars_yloss_fix<-slr_rename_h1_fix(hap1_yloss)
summary_yloss_pars_fix<-summarisebyregion(pars_yloss_fix,n_yloss)
